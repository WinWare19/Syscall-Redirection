...
BOOLEAN __stdcall ExtractStackSpace(LPBYTE, WORD*);

BOOLEAN __stdcall RedirectSystcallStubByName(LPCWSTR, LPCSTR, LPCSTR, STUB_HANDLE*);
BOOLEAN __stdcall RedirectSystcallStub(LPCWSTR, LPCSTR, DWORD, WORD, STUB_HANDLE*);
...
BOOLEAN __stdcall LookupExportItemByName(HMODULE module_base, LPCSTR export_name, INT name_length, LPBYTE* export_address) {
	...
	IMAGE_NT_HEADERS32* nt_hdr = (IMAGE_NT_HEADERS32*)((UINT_PTR)dos_hdr + dos_hdr->e_lfanew);
...
BOOLEAN __stdcall ExtractSyscallNumber(LPBYTE syscall_stub, DWORD* syscall_num) {
	...
	*syscall_num = *(DWORD*)(syscall_stub + 0x1);
	return *syscall_stub ? 0x1 : 0x0;
}

BOOLEAN __stdcall ExtractStackSpace(LPBYTE syscall_stub, WORD* stack_space) {
	if (!syscall_stub || !stack_space) return 0x0;
	*stack_space = *(WORD*)(syscall_stub + 0xD);
	return 0x1;
}

BOOLEAN __stdcall RedirectSystcallStub(LPCWSTR module_name, LPCSTR syscall_stub_name, DWORD new_syscall_num, WORD new_stack_space, STUB_HANDLE* stub_handle) {
	...
	WORD __stack_space = 0x0;
	if (!ExtractSyscallNumber(syscall_stub, &original_ssn) || !ExtractStackSpace(syscall_stub, &__stack_space)) return 0x0;
	...
	redirected_syscall_stub_info->stack_space = __stack_space;
	...
	if (!VirtualProtect(syscall_stub + 0x1, 0xF, PAGE_EXECUTE_READWRITE, &original_mem_protection[0x0])) goto FREE_REDIRECTED_SYSCALL_STUB;
	else {
		if (_InterlockedExchange((LONG*)(syscall_stub + 0x1), new_syscall_num) == original_ssn && 
			_InterlockedExchange16((SHORT*)(syscall_stub + 0xD), new_stack_space) == __stack_space)
			*stub_handle = (STUB_HANDLE)(~((UINT_PTR)(&redirected_syscall_stub_info->link)));
		VirtualProtect(syscall_stub + 0x1, 0xF, original_mem_protection[0x0], &original_mem_protection[0x1]);
...
BOOLEAN __stdcall RestoreSyscallStub(STUB_HANDLE stub_handle) {
	...
	WORD stack_space = syscall_stub_info->stack_space;
	...
	DWORD original_mem_protection[0x2] = { 0x0 };
	if (!VirtualProtect(syscall_stub_info->stub_address + 0x1, 0xF, PAGE_EXECUTE_READWRITE, &original_mem_protection[0x0])) goto FREE_SYSCALL_STUB_INFO;
	else {
		_InterlockedExchange((LONG*)(syscall_stub_info->stub_address + 0x1), original_snn);
		_InterlockedExchange16((SHORT*)(syscall_stub_info->stub_address + 0xD), stack_space);
		VirtualProtect(syscall_stub_info->stub_address + 0x1, 0xF, original_mem_protection[0x0], &original_mem_protection[0x1]);
		return_val = 0x1;
	}
...
BOOLEAN __stdcall RedirectSystcallStubByName(LPCWSTR module_name, LPCSTR syscall_stub_name, LPCSTR target_syscall_stub_name, STUB_HANDLE* stub_handle) {
	...
	WORD stack_space = 0x0;
	if (!ExtractSyscallNumber(target_syscall_stub, &syscall_num) || !ExtractStackSpace(target_syscall_stub, &stack_space)) return 0x0;

	return RedirectSystcallStub(module_name, syscall_stub_name, syscall_num, stack_space, stub_handle);
}
